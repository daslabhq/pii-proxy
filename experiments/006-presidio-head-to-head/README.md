...
| pii-proxy (regex only) | 48.0% | 10.6% | 17.4% | 15.5% | 21.8% | 0.03 |
| pii-proxy (model only) | 79.5% | 70.7% | **74.8%** | 71.4% | 83.6% | 134 |
| **pii-proxy (full)** | **76.8%** | **71.4%** | **74.0%** | 70.5% | **86.7%** | 134 |
...
many-to-many matching): **pii-proxy (full) 78.5% F1 vs Presidio 49.1%**
(+29.4pp). The full-vocabulary delta is +32.0pp.
...
| System | F1 | coverage |
|---|---|---|
| regex-first + ner-health (native labels) | 60.6% | 75.7% |
| regex-first + ner-health (+8 zero-shot labels) | 63.8% | 77.9% |
| ner-health (+zero-shot) first + regex | 68.3% | 77.9% |
...
1. **pii-proxy (full) beats default Presidio by +32.0pp F1** (74.0% vs
   42.0%), **+29.4pp on shared vocabulary** (78.5% vs 49.1%), and masks
   **86.7% vs 53.8%** of all PII spans. It wins 13 of 15 categories;
   Presidio keeps IP (98.4% vs 54.2%) and EMAIL is a tie — both regex
...
3. **Detector order is a result, not a footnote** (ablation table above).
   ...
   same regex earns +3.1pp coverage (83.6%→86.7%) masking digit strings the
   model misses, at ~1pp F1 (PHONE 71.5%→56.4% from its type-confused
   hits). **Library follow-ups: default the model detector ahead of loose
   regexes; longer-term, replace first-wins with confidence/type-aware span
   conflict resolution** (the ideal merge takes regex for EMAIL/IP, model
   for the rest — beating every ordering benchmarked here).
4. **Nobody is leak-safe out of distribution — we now leave 13.3%.**
   Concrete remaining gaps:
   - **IP, still**: 54% of gold IPs are IPv6; our regex is IPv4-only and
     even native `ipv4`/`ipv6` model labels only reach 53% — span-exact
     detection of long hex strings is a regex job, not a model job.
     Presidio's 98.4% proves it. Trivial fix, biggest single win left.
