...
- [`006-presidio-head-to-head/`](006-presidio-head-to-head/) — **First out-of-distribution, pipeline-level benchmark**: pii-proxy vs Microsoft Presidio on ai4privacy pii-masking-300k (English, n=1000, neither system tuned on it). `pii-proxy-ner` (the exp-005 flagship) + regex backstop: **74.0% F1 / 86.7% coverage vs Presidio 42.0% / 53.8%** — +29.4pp on shared vocabulary, wins 13/15 categories. Key findings: training-vocabulary breadth beats in-domain polish OOD (flagship 74.8% vs healthcare model 68.8%, gap concentrated in categories only the flagship trained on); detector order is worth 4.5pp F1 under the first-wins merge (type-aware span conflict resolution is the highest-leverage library fix); IPv6 regex gap remains our biggest single miss (Presidio 98.4% vs our 54%). Includes `verify.py` adversarial audit of the methodology itself.
